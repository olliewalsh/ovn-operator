...
if [[ "$(hostname)" != "{{ .SERVICE_NAME }}-0" ]]; then
    ...
    OPTS="--db-${DB_TYPE}-cluster-remote-addr={{ .SERVICE_NAME }}-0.{{ .SERVICE_NAME }}.openstack.svc.cluster.local --db-${DB_TYPE}-cluster-remote-port=${RAFT_PORT}"
fi
/usr/local/bin/start-${DB_TYPE}-db-server --db-${DB_TYPE}-election-timer={{ .OVN_ELECTION_TIMER }} \
{{- if .TLS }}
--ovn-${DB_TYPE}-db-ssl-key=/etc/pki/tls/private/ovn_dbs.key \
--ovn-${DB_TYPE}-db-ssl-cert=/etc/pki/tls/certs/ovn_dbs.crt \
